...
require_once 'Mysql.php'; //这里单独使用的时候有用
...
class Model {
    ...
    public function __construct($tb = null) {
		...
		if($tb)
			$this->table = $tb;
        self::$db = new Mysql(L_SERVER, L_USER,L_PASS,L_DB,L_PORT);
	...
	public function getBetween($id){
		...
		if(isset($data_prev_page[0]))
			$data_cur_page[0]['prev'] = $data_prev_page[0];
		else
			$data_cur_page[0]['prev'] = null;
		if(isset($data_next_page[0]))
			$data_cur_page[0]['next'] = $data_next_page[0];
		else
			$data_cur_page[0]['next'] = null;
